...
def get_tiling(model: str, kernel: str, src: str) -> list:
...
def get_baseline_config_num(model: str, kernel: str, logpath: str) -> int:
            ...
            for py in listdir(kernel_path):
                ...
                try:
                    cur_kernel_dict["reads_list"] = f(
                        sorted(
                            zip(reads, total_bytes[: len(reads)]), key=lambda x: x[0][0]
                        )
                    )
